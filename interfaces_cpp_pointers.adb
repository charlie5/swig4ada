...
package body Interfaces_CPP_Pointers is
...
-- tbd:            T.all := S.all;
   ...
   begin
      if S = null or else T = null then
         ...
         while L > 0 loop
-- tbd:            T.all := S.all;
-- tbd:            exit when T.all = Terminator;
            Increment (T);
            Increment (S);
            L := L - 1;
         end loop;
   ...
   begin
      if Ref = null then
         ...
         loop
-- tbd:            exit when P.all = Terminator;
            H := Index_Base'Succ (H);
            Increment (P);
         end loop;
         ...
         begin
            null;
-- tbd:      return To_PA (Ref).all;
         end;
      end if;

      return dummy : Element_Array (Index'Succ (Index'First) .. Index'First);
