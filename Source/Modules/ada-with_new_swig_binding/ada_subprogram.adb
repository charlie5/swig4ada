...
with ada_type.elementary.an_access;
...
--        log ("ada_subprogram.depends_on ~ Self.Name: '" & self.name & "'     a_Type.Name: '" & a_Type.Name & "'");
      ...
      if self.is_Function then
         if        self.return_Type.all'Access = a_Type
           or else self.return_Type.depends_on (a_Type)
         then
            return True;
         end if;
      end if;

      return depends_on (self.Parameters, a_Type);
   end depends_on;
...
--        log ("ada_subprogram.depends_on ~ Self.Name: '" & self.name & "'     a_Type.Name: '" & a_Type.Name & "'");
      ...
      if self.is_Function then
         if        self.return_Type.declaration_Package = a_Package
           or else self.return_Type.depends_on (a_Package)
         then
            return True;
         end if;
      end if;

      return depends_on (self.Parameters, a_Package);
   end depends_on;
   ...
   begin
      for Each in the_required_Types'Range loop
         if the_required_Types (Each).resolved_type.all in ada_type.elementary.an_access.Item'Class then
            return True;
         end if;
      end loop;
   ...
   begin
      ...
      if False then -- self.is_Constructor then
