...
is
   ...
   is
   begin
      return c_function.Item'(c_Declarable.Item with name               => the_Name,
   ...
   overriding function required_Types (Self : access Item) return c_declarable.c_Type_views
   is
      use c_Declarable;
      use c_Parameter;
   begin
   ...
   overriding function  depended_on_Declarations (Self : access Item) return c_Declarable.views
   is
      use c_Declarable;
      use c_Parameter;
   begin
   ...
   overriding function depends_on (Self : access Item;   a_Declarable : in     c_Declarable.view) return Boolean
   is
      use c_Parameter;
   begin
--        log ("c_function.depends_on ~ Self.Name: '" & Self.name & "'     a_Declarable.Name: '" & a_Declarable.Name & "'");
   ...
   overriding function depends_directly_on (Self : access Item;   a_Declarable : in     c_Declarable.view) return Boolean
   is
      use c_Parameter;
   begin
--        log ("c_function.depends_on ~ Self.Name: '" & Self.name & "'     a_Declarable.Name: '" & a_Declarable.Name & "'");
   ...
   begin
      ...
      if Self.is_Constructor then
         ...
         if declaration_Package.models_an_interface_Type then
            append (the_Source,  NL & "   pragma Convention (CPP, ");

         elsif declaration_Package.models_a_virtual_cpp_Class then
