...
--           log ("myName: '" & self.my.Name & "'");
      ...
      if self.my.nameSpace = null then
         log ("NULL nameSpace");
      end if;


      if self.my.nameSpace.Name = "std" then
         return self.my.Name;
      else
...
--     end;
   ...
   begin
      ...
      if self.nameSpace.Kind = header_Import then
...
--        log ("c_type.depended_on_Declarations ~ Self.Name: '" & self.name & "'");
      ...
      case self.my.c_type_Kind is
         ...
         when array_Type =>
            ...
            return self.my.element_Type.all'Access & self.my.element_Type.depended_on_Declarations;
               ...
               for Each in 1 .. self.my.component_Count loop
                  ...
                  then
                     ...
                     begin
                        ...
                        the_Types (Count + 1 .. Count + my_type_Dependencies'Length) := my_type_Dependencies;
...
--                       the_Types (Count) := self.my.Components (Each).my_Type.all'access;
         ...
         when function_Pointer =>
            ...
            return self.my.accessed_Function.all'Access & self.my.accessed_Function.depended_on_Declarations;

         when typedef_Function =>

            return self.my.typed_Function.all'Access & self.my.typed_Function.depended_on_Declarations;


         when type_Pointer =>

            return self.my.accessed_Type.all'Access & self.my.accessed_Type.depended_on_Declarations;


         when Typedef =>

            return self.my.base_Type.all'Access & self.my.base_Type.depended_on_Declarations;
...
--                 the_component_Type           : c_Type.view;
            ...
            begin
               for Each in the_depended_on_Declarations'Range loop
...
--                    end if;
         ...
         when function_Pointer =>
            ...
            return    self.my.accessed_Function.all'Access & self.my.accessed_Function.directly_depended_on_Declarations;
--              return    (1 => self.my.accessed_Function.all'access);

         when typedef_Function =>

            return    self.my.typed_Function.all'Access & self.my.typed_Function.directly_depended_on_Declarations;
...
--              end if;
         ...
         when c_Class =>
            ...
            for Each in 1 .. self.my.component_Count loop
               ...
               if self.my.Components (Each).bit_Field /= -1 then
                  --  log ("RRRRRRRRRRRRRRRRR: '" & self.my.Components (Each).my_Type.Name & "'");

                  if self.my.Components (Each).my_Type.qualified_Name = "interfaces.c.Int" then
                     self.my.requires_Interfaces_C_Int_use := True;

                  elsif self.my.Components (Each).my_Type.qualified_Name = "interfaces.c.Unsigned" then
                     self.my.requires_Interfaces_C_Unsigned_use := True;

                  elsif self.my.Components (Each).my_Type.qualified_Name = "interfaces.c.unsigned_Char" then
                     self.my.requires_Interfaces_C_Unsigned_use := True;

                  elsif self.my.Components (Each).my_Type.qualified_Name = "interfaces.c.extensions.bool" then
         ...
         when Enum =>
            ...
            begin
               ...
               for Each in 1 .. Natural (Length (self.my.Literals)) loop
                  if to_Lower (to_String (Element (self.my.Literals,  Each).Name))  =   to_Lower (to_String (self.my.Name)) then
                     Clash_found := True;
                  end if;
               end loop;
...
--                 bit_Count     : Natural           := 0;
   ...
   is
      ...
      the_Count : Natural := 0;
   begin
      for Each in 1 .. Natural (Length (self.my.nameSpace.Subprograms)) loop
         if Element (self.my.nameSpace.Subprograms,  Each).is_Virtual then
   ...
   begin
      for Each in 1 .. Natural (Length (self.my.base_Classes)) loop
         the_Count := the_Count  +  total_virtual_member_function_Count (Element (self.my.base_Classes,  Each));
      end loop;
