...
--        standard_Package : constant gnat_Package.view := new_gnat_Package ("standard",
--                                                                           parent => null);    -- old
      Self             : constant View
        := new gnat_Language.item'(base_Language with
                                   ...
                                   Package_interfaces_c_strings
                                                         => <>,
                                   --                                                       Package_interfaces_c_extensions => <>,
...
--                                     all_types_Container    => new all_types_Container.item,
                                   a_namespaces           => namespace_sets.empty_Set,

                                   --                                          current_module_Package           => null,

--                                     current_Node           => null_Doh,

--                                     current_Package        => null,
--                                     current_class_Record   => null,

--                                     current_class_swig_classType => null_unbounded_String,
--                                     current_class_swig_Type      => null_unbounded_String,

--                                     all_Packages             => swig_type_package_Maps.empty_Map,
--                                     standard_Package         => standard_Package,
--                                     namespace_Package        => null,
--                                     binding_Package          => null,
--                                     types_Package            => null,
--                                     conversions_Package      => null,
...
--        swig_Package                    : constant gnat_Package.view := old_demand_Package (Self, named => "swig");
--        interfaces_Package              : constant gnat_Package.view := old_demand_Package (Self, named => "interfaces");
--        interfaces_C_Package            : constant gnat_Package.view := old_demand_Package (Self, named => "interfaces.c");
--        interfaces_C_strings_Package    : constant gnat_Package.view := old_demand_Package (Self, named => "interfaces.c.strings");
--        --        interfaces_C_extensions_Package : gnat_Package.view := old_demand_Package (Self, named => "interfaces.c.extensions");
--        system_Package                  : constant gnat_Package.view := old_demand_Package (Self, named => "system");
--        system_crtl_Package             : constant gnat_Package.view := old_demand_Package (Self, named => "system.crtl");
   ...
   begin
      ...
      --  ada Packages
      --
      Self.Package_standard                := new_ada_Package (name => "standard",   parent => null);
      Self.Package_system                  := new_ada_Package (name => "system",     parent => Self.Package_standard);

      Self.Package_interfaces              := new_ada_Package (name => "interfaces", parent => Self.Package_standard);
      Self.Package_interfaces_c            := new_ada_Package (name => "c",          parent => Self.Package_interfaces);
      Self.Package_interfaces_c_strings    := new_ada_Package (name => "strings",    parent => Self.Package_interfaces_c);
      --        Self.Package_interfaces_c_extensions := new_ada_Package (name => "extensions", parent => Self.Package_interfaces_c);

      Self.Package_swig                    := new_ada_Package (name => "swig",       parent => Self.Package_standard);
      Self.Package_swig_pointers           := new_ada_Package (name => "pointers",   parent => Self.Package_swig);
      ...
      begin
         ...
         declare
            ...
            begin
               ...
               begin
                  ...
                  if jjj then
                     log (to_String (level_1_pointer_subType.qualified_Name) & "'");
                     raise Program_Error;
                  end if;
         ...
         begin
            ...
            --  long integers
            --
--              the_ada_Type := ada_Type.composite.an_array.new_Item (Self.Package_swig,  +"long_Array",
--                                                                    the_array_dimension_upper_Bounds => (1 => unConstrained),
--                                                                    element_type                     => Self.fetch_ada_Type (+"interfaces.c.long")).all'access;
--              Self.name_Map_of_ada_type.insert (the_ada_Type.qualified_Name, the_ada_Type);
--
--
--              the_ada_Type := ada_Type.composite.an_array.new_Item (Self.Package_swig,  +"unsigned_long_Array",
--                                                                    the_array_dimension_upper_Bounds => (1 => unConstrained),
--                                                                    element_type                     => Self.fetch_ada_Type (+"interfaces.c.unsigned_long")).all'access;
--              Self.name_Map_of_ada_type.insert (the_ada_Type.qualified_Name, the_ada_Type);
            ...
            add_associated_swig_package_Types_for (Self.fetch_ada_Type (+"interfaces.c.wchar_t"),
                                                   c_type_name => "wchar_t");
--              add_associated_swig_package_Types_for (Self.fetch_ada_Type (+"swig.pointers.wchar_t_Pointer"),
--                                                     c_type_name => "wchar_t_Pointer");
            ...
            --              do_level_3_indirection:   -- caters for 'char***'
            --              declare
            --                 c_type_Name             : String := "chars_ptr";
            --                 the_ada_array_Type      : ada_Type.view;
            --                 level_3_pointer_Type    : ada_Type.view;
            --                 level_3_pointer_subType : ada_Type.view;
            --              begin
            --                 -- array
            --                 --
            --                 the_ada_array_Type := ada_Type.composite.an_array.new_Item (Self.Package_swig_pointers,  +c_type_Name & "_pointer_Array",
            --                                                                             the_array_dimension_upper_Bounds => (1 => unConstrained),
            --                                                                             element_type                     => level_1_pointer_subType).all'access;
            --                 Self.name_Map_of_ada_type.insert (the_ada_array_Type.qualified_Name, the_ada_array_Type);
            --
            --
            --                 -- pointer
            --                 --
            --                 level_2_pointer_Type := ada_Type.elementary.an_access.to_type.new_Item (Self.Package_swig_pointers, +"c_" & c_type_Name & "_pointer_Pointer.Pointers",
            --                                                                                         accessed_type => level_1_pointer_subType).all'access;
            --                 Self.name_Map_of_ada_type.insert (level_2_pointer_Type.qualified_Name, level_2_pointer_Type);
            --
            --
            --                 level_2_pointer_subType := ada_type.a_subtype.new_subType (Self.Package_swig_pointers, +c_type_Name & "_pointer_Pointer",
            --                                                                            base_type => level_2_pointer_Type).all'access;
            --                 Self.name_Map_of_ada_type.insert (level_2_pointer_subType.qualified_Name, level_2_pointer_subType);
            --              end do_level_2_indirection;
...
--         Self.associate (Self.fetch_ada_Type (+"swig.pointers.std_String_array"), Self.fetch_c_Type   (+"std::string[]"));

         declare   -- tbd: ?
            kkk : ada_type.view := Self.fetch_ada_Type (+"swig.pointers.std_string_Pointer");
         begin
            null;
         end;
...
--           Self.associate (Self.fetch_ada_Type (+"swig.pointers.wchar_t_pointer_Pointer"),    Self.fetch_c_Type   (+"wchar_t**")            );
      ...
      --  old ...
      --
--        Self.all_types_Container.Owner_is (Self);
      Self.add_Namespace (+"std");

--        insert (Self.all_Packages,  to_unbounded_String ("standard"), standard_Package);

--        standard_Package               .is_Core;
--        swig_Package                   .is_Core;
--        interfaces_Package             .is_Core;
--        interfaces_C_Package           .is_Core;
--        interfaces_C_strings_Package   .is_Core;
--        --        interfaces_C_extensions_Package.is_Core;
--        system_Package                 .is_Core;
--        system_crtl_Package            .is_Core;
