      ...
      if hex_Index = 0 then
         for Each in self'Range loop
            case Self (Each) is
               ...
               when others     => log ("bad numeric character: '" & Self (Each) & "'   '" & Self & "'");   raise Constraint_Error;
...
--           log ("result: " & Image (result));
         ...
         for Each in hex_Index + 2 .. self'Last loop
--              log ("hex ~ result: " & huge_Integer'image (to_huge_Integer (result)) & "    curr Char: '" & Self (Each) & "'");
            ...
            case Self (Each) is
               ...
               when 'a' .. 'f' => Result := Result * to_Integer (16) + to_Integer (Character'Pos (Self (Each)) - Character'Pos ('a') + 10);
               when ')'        => exit;
               when others     => log ("bad hex character: '" & Self (Each) & "'   '" & Self & "'");   raise Constraint_Error;
            end case;
         end loop;
...
--      log ("'to_ada_Identifier' ~   the_Identifier (rid namespace): '" & the_Identifier & "'");
      ...
      if is_reserved_Word (the_Identifier) then
