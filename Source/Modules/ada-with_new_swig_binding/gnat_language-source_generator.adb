...
with ada_Type.elementary.an_access.to_type.interfaces_c_pointer;
with ada_Type.elementary.an_access.to_subProgram;
with ada_Type.composite.an_array;
with ada_Type.composite.a_record;
with ada_Type.elementary.scalar.discrete.enumeration;
...
--     use all_types_Container;
   ...
   --     function required_Types (Self : access ada_Type.Item;    the_Gnat_Lang : access gnat_Language.item) return ada_Type.views
   --     is
   --        use ada_Type;
   --     begin
   --
   --        case Self.c_type_Kind is
   --
   --           when array_Type =>
   --
   ...
   --                    log ("" & Self.my_view_type.qualified_Name & "     " & Self.Components (Each).my_Type.resolved_Type.qualified_Name);
   --                    if Self.Components (Each).my_Type.resolved_Type /= my_view_Type (Self) then
   --                       Count := Count + 1;
   --                       the_Types (Count) := Self.Components (Each).my_Type.resolved_Type;
   ...
   --
   --              return Self.accessed_Function.required_Types;
   --
   --
   --           when type_Pointer =>
   --
   ...
   --
   --              return (1 => the_gnat_Lang.name_Map_of_ada_type.Element (+"system.Address"));
   --
   --
   --              --           when Unknown =>
   --              --              raise Program_Error;
...
--     function required_Types (Self : access gnat_Type.Item'Class;    the_Gnat_Lang : access gnat_Language.item) return gnat_Type.views
--     is
--     begin
--
--        case Self.c_type_Kind is
...
--           when c_Class =>
--              declare
--                 the_Types : gnat_Type.views (1 .. Self.component_Count);
--                 Count     : Natural := 0;
--              begin
--                 for Each in the_Types'Range loop
--  --                    log ("" & Self.my_view_type.qualified_Name & "     " & Self.Components (Each).my_Type.resolved_Type.qualified_Name);
...
--           when virtual_class_Pointer =>
--
--              return (1 => the_gnat_Lang.all_types_Container.fetch_Type ("system.Address"));
--
--
--              --           when Unknown =>
--              --              raise Program_Error;
...
--     function depends_on (Self   : access gnat_Type.Item'Class;
--                          a_Type : in     gnat_Type.view;
--                          the_Gnat_Lang : access gnat_Language.item)    return Boolean
--     is
--        my_Depends : constant gnat_Type.views := required_Types (Self, the_Gnat_Lang);
--     begin
--
--        for Each in my_Depends'Range loop
--           if my_Depends (Each) = a_Type then
...
--     end depends_on;
   ...
   begin
      ...
      if not is_Empty (base_Classes) then
         ...
         begin
            while has_Element (Cursor) loop
               ...
               if Cursor /= First (base_Classes) then
         ...
         if Self.is_tagged_Type  or  Length (base_Classes) > 1 then
         ...
         elsif Self.virtual_member_function_Count > 0 then
...
--     function c_Class_public_declaration_Text (Self : access gnat_Type.Item) return String
--     is
--        use ada.Containers;
--
--        the_Source              : unbounded_String;
--        union_variant_type_Name : constant unbounded_String := Self.Name & "_variant";
--
--        base_Classes : constant gnat_type.Vector := Self.base_Classes;
--     begin
--
--        append (the_Source,  NL  &  "type "  &  Self.Name);
--
...
--
--           declare
--              Cursor : gnat_type.Cursor := First (base_Classes);
--           begin
--              while has_Element (Cursor) loop
--
--                 if Cursor /= First (base_Classes) then
...
--           end;
--
--           if is_tagged_Type (Self)  or  Length (base_Classes) > 1 then
--              append (the_Source,  " with private");
--           end if;
--
--        else
...
--
--        return to_String (the_Source);
--     end c_Class_public_declaration_Text;
   ...
   begin
      ...
      if Self.requires_Interfaces_C_Int_use then
         append (the_Source, "   use type interfaces.C.int;"   & NL & NL);
      end if;

      if Self.requires_Interfaces_C_Unsigned_use then
         append (the_Source, "   use type interfaces.C.Unsigned;"   & NL & NL);
      end if;

      if Self.requires_Interfaces_C_Unsigned_Char_use then
         append (the_Source, "   use type interfaces.C.Unsigned_Char;"   & NL & NL);
      end if;

      if Self.requires_Interfaces_C_Extensions_bool_use then
      ...
      if Self.is_Union then
         ...
         declare
            --  use gnat_Variable.array_bounds_Vectors, ada.Containers;
         ...
         begin
            for Each in 1 .. Self.component_Count loop
               ...
               if Each /= Self.component_Count then
                  append (the_Source,  ", ");
               end if;
      ...
      if not is_Empty (base_Classes) then
         ...
         --           if Self.is_tagged_Type  or  base_Classes.Length > 1 then
      ...
      if not Self.is_interface_Type then
         declare
         ...
         begin
            ...
            --
            if Self.component_Count = 0 then
               ...
               begin
                  for Each in 1 .. Self.component_Count loop
                     ...
                     --                          then
                     --
                     --                             if the_component.my_type.my.declaration_Package.is_Core then
   ...
   is
      ...
      function "<" (Left, Right : in ada_Type.elementary.scalar.discrete.enumeration.enum_Literal) return Boolean is begin   return Value (Left.Value) < Value (Right.Value);   end "<";
   ...
   begin
      ...
      if is_Empty (Self.Literals) then
      ...
      if is_Empty (the_Literals) then
         append (the_Source,  " nil => 0");
      else
         for Each in 1 .. Natural (Length (the_Literals)) loop
            if not (         Each > 1
                    and then Element (the_Literals,  Each).Value = Element (the_Literals,  Each - 1).Value)   -- skip any with duplicate values.
            then
               if Each > 1 then
...
--     function Enum_declaration_Text (Self : access gnat_Type.Item) return String
--     is
--        use gnat_type.enum_literal_Vectors;
--        the_Source : unbounded_String;
--     begin
--        append (the_Source,  "   type " & Self.Name & " is (");
--
...
--           for Each in 1 .. Natural (Length (Self.Literals)) loop
--              if not (         Each > 1
--                      and then Element (Self.Literals,  Each).Value = Element (Self.Literals,  Each - 1).Value)   -- skip any with duplicate values.
--              then
--                 if Each > 1 then
--                    append (the_Source,  "," & NL);
--                 end if;
--
--                 append (the_Source,  "   " & element (Self.Literals,  each).Name);
--              end if;
--           end loop;
--        end if;
--
...
--                 end if;
--
--                 append (the_Source,  "   " & element (Self.Literals, each).Name  &  " => "  &  Image (element (Self.Literals, each).Value));
--              end if;
--           end loop;
--        end if;
--
...
--     end Enum_declaration_Text;
   ...
   --              append (the_Source,  Self.my.accessed_Function.specification_Source (declaration_package => Self.my.declaration_Package,
   ...
   --                and then Self.accessed_Type.depends_on (gnat_Type.view (Self))                 -- if that is what we reference
   --              then                                                                             --
   ...
   begin
      ...
      if Self.all in ada_Type.composite.a_record.item'Class then
         declare
            Self : constant ada_Type.composite.a_record.view := ada_Type.composite.a_record.view (representation_Text.Self);
         begin

            --            append (the_Source,  ada_Record_private_declaration_Text (ada_Type.composite.a_record.view (Self)));

            if Self.contains_bit_Fields then
               append (the_Source,
                       NL & NL
         ...
         --        elsif Self.all in ada_Type.elementary.an_access.to_subprogram.item'Class then
         --           return to_String (NL & "pragma convention (C, " & Self.Name & ");");
   ...
   begin
         ...
         begin
            ...
            while has_Element (Cursor) loop
               declare
                  ...
                  using_access_Mode     : constant Boolean :=          the_Parameter.my_Type.resolved_Type.all in ada_type.elementary.an_access.to_subprogram.item'Class
               ...
               begin
                  ...
                  then
                     ...
                     then
                        --  append (the_Mode, "access");        -- for non-virtual C struct's and class's.
                  ...
                  --                    and then the_Parameter.my_Type.c_type_Kind /= c_Class
                  then
                     --                       log ("PARAM TYPE QUAL NAME: '" & the_Parameter.my_Type.qualified_Name);

                     --                       if         parameter_type_Package.is_Core
                     --                         and then the_Parameter.my_type.c_type_Kind = Enum
                     --                       then
                  ...
                  then
                     declare
                        --                          using_access_Mode     : Boolean :=          the_Parameter.my_Type.resolved_Type.c_type_Kind = type_Pointer
                        --                                                             and then to_Lower (the_Parameter.my_Type.resolved_Type.qualified_Name) /= "interfaces.c.strings.chars_ptr";
                        --
                        --                          returns_virtual_Class : Boolean :=          using_access_Mode
                        --                                                             and then     the_Parameter.my_Type.accessed_Type.declaration_Package.models_a_virtual_cpp_Class
                        --                                                             and then not the_Parameter.my_Type.accessed_Type.is_interface_Type;
                     begin
                        if using_access_Mode
                        then
                        ...
                        --                       append (the_Source,  the_Parameter.my_Type.resolved_Type.qualified_Name);
                     end;
                     --                       append (the_Source,  the_Parameter.my_Type.accessed_Type.resolved_Type.qualified_Name);
                  else
                  ...
                  if Cursor /= Last (all_Parameters) then
                     append (the_Source,  ";" & NL);
                  end if;
               end;
      ...
      if Self.is_Function then
         declare
            ...
            using_access_Mode     : constant Boolean                :=          Self.return_Type.resolved_Type.all in ada_type.elementary.an_access.to_type.item'Class
              and then to_Lower (Self.return_Type.resolved_Type.qualified_Name) /= "interfaces.c.strings.chars_ptr";
            --  returns_virtual_Class :        Boolean                 :=          using_access_Mode
            --                                                            and then     ada_Type.elementary.an_access.to_type.view (Self.return_Type).accessed_Type.declaration_Package.models_a_virtual_cpp_Class
            --                                                            and then not ada_Type.elementary.an_access.to_type.view (Self.return_Type).accessed_Type.is_interface_Type;
            function returns_virtual_Class return Boolean
            is
            begin
               if         using_access_Mode
         ...
         begin
            ...
            if declaration_Package.type_name_needs_Standard_prefix ("",  Self.return_Type.resolved_Type) then
               if namespace_Prefix = "" then
                  append (the_Source,  "standard.");
               else
                  append (the_Source,  namespace_Prefix);
            ...
            if using_access_Mode then
               if returns_virtual_Class then
                  ...
                  --                    append (the_Source, qualified_Name (Self.my_Type.resolved_Type.accessed_Type.declaration_Package) & ".Pointer");
               else
                  append (the_Source,
                          qualified_Name (ada_Type.elementary.an_access.to_type.view (Self.return_Type.resolved_Type).accessed_Type));
--                            qualified_Name (ada_Type.elementary.an_access.to_type.view (Self.return_Type.resolved_Type).accessed_Type) & "'Class");
               end if;
            else
               append (the_Source,
            ...
            if        (         return_type_Package.cpp_class_Type /= null
              ...
              --                or else (         Self.return_Type.accessed_Type /= null
              --                         and then Self.return_Type.accessed_Type.is_tagged_Type)
            then
               append (the_Source,  "'Class");   -- "JJJ_'Class");
            end if;
   ...
   begin
      ...
      then
         ...
         --           declare
         --              My : ada_Type.elementary.an_access.to_type.interfaces_c_pointer.view := ada_Type.elementary.an_access.to_type.interfaces_c_pointer.view (Self);
         --           begin
         --              append (the_Source, "   type " & my.accessed_Type.Name & "_Pointer is access all " & my.accessed_Type.qualified_Name & ";");
         --
         --  --              if         my.accessed_Type.all in ada_Type.composite.a_record.item'Class
         --  --                and then ada_Type.composite.a_record.view (my.accessed_Type).is_tagged_Type
         --  --              then
         --  --                 append (the_Source, "   package " & package_name_Prefix & "Pointers is new interfaces_cpp_Pointers (interfaces.c.size_t, "
         --  --                                                                                                                   & my.accessed_Type.qualified_Name & ", "
         --  --                                                                                                                   & associated_array_Name & ", "   -- my.accessed_Type.Name & "_Array, "
         --  --                                                                                                                   & default_Terminator & ");");
         --  --              else
         --  --                 append (the_Source, "   package " & package_name_Prefix & "Pointers is new interfaces.c.Pointers (interfaces.c.size_t, "
         --  --                                                                                                                 & my.accessed_Type.qualified_Name & ", "
         --  --                                                                                                                 & associated_array_Name & ", "   -- my.accessed_Type.Name & "_Array, "
         --  --                                                                                                                 & default_Terminator & ");");
         --  --              end if;
         ...
         begin
            ...
            append (the_Source,  specification_Source (Self => my.accessed_subProgram, declaration_package => my.declaration_Package,
                                                       using_Name          => null_unbounded_String,
                                                       namespace_Prefix    => null_unbounded_String,
      ...
      then
         ...
         begin
            if         Self.accessed_Type.all in ada_Type.composite.a_record.item'Class -- = c_Class         -- add forward declaration of a class type,
            ...
            if         Self.accessed_Type.all in  ada_Type.composite.a_record.item'Class       -- = c_Class
      ...
      elsif Self.all in ada_Type.composite.a_record.item'Class then
         declare
      ...
      elsif Self.all in ada_Type.elementary.scalar.discrete.enumeration.item'Class then
         declare
      ...
      elsif Self.all in ada_Type.a_subType.item'Class then
         declare
            Self : constant ada_Type.a_subType.view := ada_Type.a_subType.view (declaration_text.Self);
         begin
            --              if Self.base_Type.all in ada_type.elementary.an_access.to_type.item'Class then
            --                 return to_String ("   type "  &  Self.Name  &  " is access all "  &  ada_Type.elementary.an_access.to_type.view (Self.base_Type).accessed_Type.qualified_Name   &  ";");
            if Self.base_Type.all in ic_Pointer_class then
         ...
         --           when virtual_class_Pointer =>
         --
         --              append (the_Source,  "   type " & Self.Name & " is new system.Address;");
         --
         --
         --           when array_Type => null;
         --
         --  --              append (the_Source,  "   type " & Self.Name & " is array (");
         --  --
         ...
         --  --                 end if;
         --  --
         --  --                 if Self.array_Dimensions_upper_Bound (Each)  =  -1 then
         --  --                    --append (the_Source,  "Natural range <>");
         --  --                    append (the_Source,  "interfaces.C.Size_t range <>");
         --  --                 else
         --  --                    --append (the_Source,  "Natural range 0 .. " & Image (Self.my.array_Dimensions_upper_Bound (Each)));
         --  --                    append (the_Source,  "interfaces.C.Size_t range 0 .. " & Image (Self.array_Dimensions_upper_Bound (Each)));
         --  --                 end if;
         --  --
         --  --              end loop;
         --  --
         --  --              append (the_Source,  ") of aliased " & qualified_Name (Self.element_Type) & ";");
         --
         --
         --           when c_Class =>
         --              null;
         --  --              return c_Class_public_declaration_Text (ada_Type.composite.a_record.view (Self));
         --
         --
         --           when Enum =>
         --              null; -- return Enum_declaration_Text (Self);
         --
         --
         --           when function_Pointer =>
         --
         --  --              append (the_Source,  "   type " & Self.Name & " is access ");
         --  --              append (the_Source,  specification_Source (Self => Self.accessed_Function, declaration_package => Self.declaration_Package,
         --  --                                                                                using_Name          => null_unbounded_String,
         --  --                                                                                namespace_Prefix    => null_unbounded_String,
         --  --                                                                                the_gnat_lang => the_gnat_lang));
         --  --              append (the_Source,  ";");
         ...
         --           when type_Pointer =>
         --
         --  --              if         ada_Type.elementary.an_access.to_type.view (Self).accessed_Type.c_type_Kind = c_Class                              -- add forward declaration of a class type,
         --  --                and then depends_on (ada_Type.elementary.an_access.to_type.view (Self).accessed_Type,  ada_Type.view (Self),  the_Gnat_Lang)                -- if that is what we reference
         --  --              then                                                                             --
         --  --                 append (the_Source, "   type " & ada_Type.elementary.an_access.to_type.view (Self).accessed_Type.Name & ";" & NL & NL);    --
         --  --              end if;
         --  --
         --  --
         --  --              append (the_Source,  "   type " & Self.Name & " is access all "  &  ada_Type.elementary.an_access.to_type.view (Self).accessed_Type.qualified_Name);
         --  --
         --  --              if         ada_Type.elementary.an_access.to_type.view (Self).accessed_Type.c_type_Kind = c_Class
         --  --                and then ada_Type.elementary.an_access.to_type.view (Self).accessed_Type.is_tagged_Type
         --  --              then
         --  --                 append (the_Source,  "'Class ");
         --  --              end if;
         --  --
         ...
         --              | Unknown       =>
         --
         --              return  to_String ("   type " & Self.Name & " is new interfaces.c.extensions.opaque_structure_def;");
         --
         --
         --           when incomplete_Class =>
         --              return  to_String ("   type " & Self.Name & " is new interfaces.c.extensions.incomplete_class_def;");
         --
         --
         --           when standard_c_Type =>
         --              return "";
         ...
         --           when a_subType =>
         --
         --              if Self.base_Type.c_type_Kind = type_Pointer then
         --                 return to_String ("   type "  &  Self.Name  &  " is access all "  &  ada_Type.elementary.an_access.to_type.view (Self.base_Type).accessed_Type.qualified_Name   &  ";");
         --              else
         --                 return to_String ("   subtype "  &  Self.Name  &  " is "  &  Self.base_Type.qualified_Name   &  ";");
         --                 -- return to_String ("   subtype "  &  Self.my.Name  &  " is "  &  Self.my.base_Type.qualified_Name   &  ";");
   ...
   procedure emit_Spec (Self          : access ada_Package.Item'Class;
   ...
   is
      ...
      all_Variables   : constant ada_variable.Vector    := Self.Variables;
      all_Subprograms : constant ada_subprogram.Vector := Self.Subprograms;

      all_Types        : constant ada_Type.Vector        := Self.Types;
...
--        log ("");
      log ("");
      log ("'emit_Spec' -    package name: '" & Self.qualified_Name & "'");

      if Self.is_Core then -- or  Self.a_is_Unknown then
         return;
      end if;
      ...
      begin
         while has_Element (Cursor) loop
            ...
            if         Self.models_a_virtual_cpp_Class
              and then (
...
--        log ("emit 'with's");
      ...
      declare
         ...
         --           with_of_interfaces_c_pointers_Required   : Boolean          := False;        -- ditto, but for pointers.
         with_of_interfaces_c_Required            : constant Boolean          := False;

         the_Subprogram : ada_Subprogram.view;
      begin
--           log ("adding required subprogram types to context.");

         for Each in 1 .. Natural (Length (all_Subprograms)) loop

            the_Subprogram := Element (all_Subprograms,  Each);

--              log ("subprog name: '" & the_Subprogram.Name & "'");
            ...
            begin
               while has_Element (Cursor) loop
                  ...
                  then
                     the_Context.add (the_Parameter.my_Type,  is_access => True);

                  elsif the_Parameter.my_Type.all in ada_type.elementary.an_access.to_type.item'Class then
                     the_Context.add (ada_Type.elementary.an_access.to_type.view (the_Parameter.my_Type).accessed_Type, is_access => True);

                  else
                     the_Context.add (the_Parameter.my_Type, is_access => False);
                  end if;
            ...
            if the_Subprogram.is_Function then   -- add return type to context.
               declare
                  the_return_Type : constant ada_Type.view := the_Subprogram.return_Type;
               begin
--                    log ("the subprogram type: " & the_Subprogram.return_Type.qualified_Name);

                  if the_return_Type.all in ada_type.elementary.an_access.to_type.item'Class then
                     the_Context.add (ada_Type.elementary.an_access.to_type.view (the_Subprogram.return_Type).accessed_Type, is_access => True);
                  else
                     the_Context.add (the_Subprogram.return_Type, is_access => False);
                  end if;
               end;
            end if;


            --  add_to (the_Context,  Element (Self.a_Subprograms,  Each).required_Types);
         end loop;



--           log ("adding required variable types to context.");

         for Each in 1 .. Natural (Length (all_Variables)) loop
            declare
               the_Variable : constant ada_Variable.view := Element (all_Variables,  Each);
            begin
--                 log ("variable Name: '" & the_Variable.Name & "'");

               the_Context.add (the_Variable.my_Type); --, is_parameter => False); -- True);

               --  if Element (Self.a_Variables,  Each).my_Type.qualified_Name = "interfaces.c.char_Array" then
               if the_Variable.my_Type.resolved_Type.qualified_Name = "interfaces.c.char_Array" then
...
--           log ("adding own declared types to context.");
         for Each in 1 .. Natural (Length (all_Types))
         loop
            declare
               the_Type : constant ada_Type.view := Element (all_Types,  Each);
            begin
--                 log ("doing type: '" & the_Type.qualified_Name & "'");

               --                 the_Context.add (required_Types (the_Type, the_Gnat_Lang)); --, is_parameter => false);
               --                 the_Context.add (the_Type.required_Types); --, is_parameter => false);


               if the_Type.all in ada_Type.composite.a_record.Item'Class then
                  declare
                     the_record_Type    : constant ada_Type.composite.a_record.view := ada_Type.composite.a_record.view (the_Type);
                     the_component_Type : ada_Type.view;
                  begin
                     for Each in 1 .. the_record_Type.component_Count loop
                        ...
                        if not (         the_component_Type.all in ada_type.a_subType.item'Class                                      -- ignore Self referential pointer
                                and then ada_type.a_subType.view (the_component_Type).base_Type.all in ada_Type.elementary.an_access.to_type.item'Class
                                and then ada_Type.elementary.an_access.to_type.view (ada_type.a_subType.view (the_component_Type).base_Type).accessed_Type = the_record_Type.all'Access)
                        then

                           if         the_component_Type.all in ada_type.a_subType.item'Class                                         -- ignore Self referential pointer
                             and then ada_type.a_subType.view (the_component_Type).base_Type.all in ada_Type.elementary.an_access.to_type.item'Class
                           then
                              the_Context.add (ada_Type.elementary.an_access.to_type.view (ada_type.a_subType.view (the_component_Type).base_Type).accessed_Type, is_access => True);
                           else
                  ...
                  the_Context.add (the_Type.context_required_Types); --, is_parameter => false);
               end if;



               --                 if   the_Type.all in c_type_Kind = opaque_Struct
               --                   or the_Type.c_type_Kind = incomplete_Class
               --  --                   or the_Type.c_type_Kind = Unknown
               --                 then
               --                    with_of_interfaces_c_extensions_Required := True;
               --
               --                 elsif the_Type.c_type_Kind = array_Type then
               --                    with_of_interfaces_c_Required := True;
               --
               --                 elsif the_Type.my_array_Type /= null then
               --                    with_of_interfaces_c_pointers_Required := True;
               --                 end if;
            end;
         end loop;


--           log ("adding base class types to context source.");   -- old
         if Self.cpp_class_Type /= null then
            declare
               the_base_Classes : constant ada_Type.vector := Self.cpp_class_Type.base_Classes;
            begin
               for Each in 1 .. Natural (Length (the_base_Classes)) loop
                  the_Context.add (Element (the_base_Classes, Each), is_access => False);

                  --  append (spec_Source,   "with "   &  Element (the_base_Classes, Each).declaration_package.qualified_Name   &   ";"   &   NL);
                  --  append (spec_Source,   "with "   &  namespace_Prefix  &  Element (Self.cpp_class_Type.base_Classes,  Each).declaration_package.qualified_Name   &   ";"   &   NL);
               end loop;
            end;
         end if;
...
--           log ("adding context to package source.");
         append (spec_Source,
                 to_Source (the_Context,
                            Self.all'Access,
                            null_unbounded_String)); -- namespace_Prefix));


         if with_of_swig_Required then                   -- ensure Swig is with'ed in case of i.c.char_Array to Swig.indefinate_Arrsy transformation.
            append (spec_Source,   "with Swig;" &  NL);                          -- tbd: may result in a redundant withing of 'Swig' package.
         end if;

         if with_of_interfaces_c_extensions_Required then -- ensure interfaces.C.extensions is with'ed in case of opaque struct's or imcomplete classes.
            append (spec_Source,   "with interfaces.C.extensions;" &  NL);       -- tbd: may result in a redundant withing of package.
         end if;

         if with_of_interfaces_c_Required then
            append (spec_Source,   "with interfaces.C;" &  NL);
         end if;

         append (spec_Source,   "with interfaces.c.Pointers;"   &  NL);   -- temp
         --           append (spec_Source,   "with interfaces_cpp_Pointers;" &  NL);   -- temp
         append (spec_Source,   "with interfaces.c.Strings;"    &  NL);   -- temp
         append (spec_Source,   "with System;"                  &  NL);   -- temp
         --           append (spec_Source,   "with Swig.Pointers;"           &  NL);   -- temp

         --           if with_of_interfaces_c_pointers_Required then
         --              append (spec_Source,   "with interfaces.C.Pointers;" &  NL);
         --           end if;



         --           append (spec_Source,   "with interfaces.C.strings;" &  NL);

         if         Self.models_a_virtual_cpp_Class
           and then not Self.models_an_interface_Type
         then
      ...
      if Self.is_global_Namespace then
         append (spec_Source,   "package " & Self.qualified_Name & " is" & NL & NL);
      else
         append (spec_Source,   "package " & Self.qualified_Name & " is" & NL & NL);
...
--        log ("emit main types");
      ...
      begin
         ...
         --
         --              Cursor      : ada_Type.Cursor := First (all_Types);
         ...
         --                       --                       log ("testing: " & the_Type.qualified_Name & "    with    " & Element (Test_Cursor).qualified_Name);
         --
         --                       if         the_Type.depends_directly_on (Element (test_Cursor))
         --  --                         and then not (         the_Type.c_type_Kind               = type_Pointer     -- guard against classes which contain a
         --  --                                       and then ada_Type.elementary.an_access.to_type.view (the_Type).accessed_Type.c_type_Kind = c_Class          -- pointer to themselves as a member variable.
         --  --                                       and then depends_on (ada_Type.elementary.an_access.to_type.view (the_Type).accessed_Type,  the_Type, the_Gnat_Lang)   )    --
         --  --                       --                         and then not (         each_Type.c_type_Kind               = c_Class          -- guard against classes which contain a
         --  --                       --                                       and then Element (test_Cursor).c_type_Kind   = type_Pointer     -- pointer to themselves as a member variable.
         --  --                       --                                       and then Element (test_Cursor).accessed_Type = each_Type   )    --
         --                       then
         --                          --                          log ("swapping");
         --                          swap (all_Types,  Cursor, test_Cursor);
         --                          Cursor := First (all_Types);
         --                          exit primary;
         --                       end if;
         --
         --                       next (test_Cursor);
         ...
         --                    next (Cursor);
         --
         --                    if not has_Element (Cursor) then        -- sorting is complete
         --                       is_Sorted := True;
         --                    end if;
         --
         --                 end loop primary;
         ...
         --                       --                       log ("testing: " & the_Type.qualified_Name & "    with    " & Element (Test_Cursor).qualified_Name);
         --
         --                       if         depends_on (the_Type,  Element (test_Cursor),  the_Gnat_Lang)
         --                         and then not (         the_Type.c_type_Kind               = type_Pointer     -- guard against classes which contain a
         --                                       and then ada_Type.elementary.an_access.to_type.view (the_Type).accessed_Type.c_type_Kind = c_Class          -- pointer to themselves as a member variable.
         --                                       and then depends_on (ada_Type.elementary.an_access.to_type.view (the_Type).accessed_Type,  the_Type, the_Gnat_Lang)   )    --
         --                       --                         and then not (         each_Type.c_type_Kind               = c_Class          -- guard against classes which contain a
         ...
         --           for Each in 1 .. Natural (Length (Self.a_Types)) loop
         --
         --              the_Type := Element (Self.a_Types,  Each);
         --
         --              if the_Type.c_type_Kind = c_Class then
         --                 pragma assert (the_cpp_class_Record = null);          -- there should be only one c_Class per package
         --                 the_cpp_class_Record := the_Type;
         --              end if;
         --           end loop;
         ...
         --              the_Type := Element (Self.a_Types,  Each);
         --
         --              if      (         the_Type.c_type_Kind   = type_Pointer
         --                       and then the_Type.accessed_Type = the_cpp_class_Record)
         --              then
         --                 the_cpp_class_record_View := the_Type;
         --              end if;
         --
         --              if      (         the_Type.c_type_Kind  = array_Type
         --                       and then the_Type.element_Type = the_cpp_class_Record)
         --              then
         --                 the_cpp_class_record_Array := the_Type;
         --              end if;
         --           end loop;


         --           for Each in 1 .. Natural (Length (all_Types)) loop
         --              the_Type := Element (all_Types,  Each);
         ...
         loop
            ...
            declare
            begin
               --              if not (         the_Type.c_type_Kind = array_Type                                                            -- ignore any array types whose
               --                      and then (         (         the_Type.element_Type.c_type_Kind                        = c_Class       -- element type is a C class and which
               --                                          and then the_Type.element_Type.pure_virtual_member_function_Count > 0      )      -- is abstract, or whose
               --                                or else (         the_Type.element_Type.c_type_Kind = array_Type                           -- element type is an array, which
               --                                         and then the_Type.element_Type.is_Unconstrained        )                    ))   -- is unconstrained.
               ...
               then
                  ...
                  begin
                     if the_representation_Text /= "" then
         ...
         --           if the_cpp_class_Record /= null then
         --
         --              append (spec_Source,          the_cpp_class_Record.use_type_Text  & NL & NL);
         --
         --              append (spec_Source,          the_cpp_class_Record.declaration_Text     & NL & Nl);        -- nb: this is deferred, since it may
         --              append (spec_Source_private,  the_cpp_class_Record.representation_Text  & NL & NL);        --     rely on prior types.
         --
         --              --if not the_cpp_class_record_View.is_abstract then
         --              if not the_cpp_class_Record.is_interface_Type then
         --                 append (spec_Source,          the_cpp_class_record_Array.declaration_Text     & NL & Nl);
         --                 append (spec_Source_private,  the_cpp_class_record_Array.representation_Text  & NL & NL);
         --              end if;
         --
         --              append (spec_Source,          the_cpp_class_record_View.declaration_Text     & NL & Nl);
         --              append (spec_Source_private,  the_cpp_class_record_View.representation_Text  & NL & NL);
         --           end if;
      ...
      --           append (spec_Source,   "   type Pointer is new system.Address;"                     & NL & NL);
      --           append (spec_Source,   "   function to_View    (Self : in Pointer) return View;"    & NL);
      --           append (spec_Source,   "   function to_Pointer (Self : in View)    return Pointer;" & NL);
      --        end if;
      ...
      --  append (spec_Source,  "   use type interfaces.C.int;"                           & NL & NL);
...
--        log ("emit variables");
      ...
      for Each in 1 .. Natural (Length (all_Variables)) loop
         declare
            use ada_Variable.array_bounds_Vectors,  ada.Containers;

            the_Variable : constant ada_Variable.view := Element (all_Variables,  Each);
            --  type_Class   : gnat_Package.view  := the_Variable.my_Type.declaration_Package;
            type_Class   : access ada_Package.item'Class  := the_Variable.my_Type.resolved_Type.declaration_Package;
         ...
         begin
            ...
            if         the_Variable.is_Constant
              ...
              --                and then the_Variable.my_Type.resolved_Type.qualified_Name  = "interfaces.c.int"
              and then the_Variable.Value                                /= ""
            then
               append (spec_Source,  "constant ");

               --              elsif the_Variable.is_class_Pointer then
               ...
               if the_Variable.is_Constant then
                  append (spec_Source,  "constant ");
               end if;


               if   the_Variable.is_Pointer   and   the_Variable.my_Type.resolved_Type.all in ada_type.elementary.scalar.discrete.enumeration.item'Class then
                  append (spec_Source,  " access ");
               end if;


               if  the_Variable.is_class_Pointer --   and   not type_Class.is_Core   and   the_Variable.my_Type.Kind.all not in swig_Type.item'Class)
               --  or (the_Variable.is_class_Pointer  and   not type_Class.is_Core   and   the_Variable.my_Type.Kind.all not in swig_Type.item'Class)
               then
               ...
               if Self.type_name_needs_Standard_prefix (to_String (the_Variable.Name),  the_Variable.my_Type.resolved_Type) then
                  --                    if namespace_Prefix = "" then
                  append (spec_Source,  "standard.");
                  --                    else
                  --                       append (spec_Source,  namespace_Prefix);
                  --                    end if;
               end if;


               --  append (spec_Source,  the_Variable.my_Type.resolved_Type.qualified_Name);
               ...
               if not is_Empty (the_variable.array_Bounds) then
                  append (spec_Source, " (");

                  for Each in 1 .. Integer (Length (the_variable.array_Bounds)) loop
                     if Each > 1 then
                        append (spec_Source, ",");
                     end if;

                     if Element (the_variable.array_Bounds, Each) = -1 then
            ...
            if the_Variable.is_Constant then
               if    the_Variable.my_Type.resolved_Type.qualified_Name = "interfaces.c.strings.chars_ptr" then
                  append (spec_Source,  " := interfaces.c.strings.new_String (""" & the_Variable.Value & """)");

               elsif the_Variable.my_Type.resolved_Type.qualified_Name = "Character" then
                  append (spec_Source,  " := '" & the_Variable.Value & "'");
               else
                  if the_variable.my_Type.resolved_Type.is_ultimately_Unsigned then
            ...
            --  if the_Variable.Value = "" then
            if not the_Variable.is_Constant then
               emit_import_Pragma :
               declare
                  ...
                  is
                  begin
                     if in_cpp_Mode then                               -- tbd: need to use proper storage class ?

                        if Self.is_Module then
                           return the_Variable.Name;
                        else
                           return "_ZN" & Image (Length (Self.Name))  & Self.Name
...
--        log ("emit subprograms");
      ...
      begin
         ...
         while has_Element (Cursor) loop
            ...
            begin
               if the_Subprogram.is_to_view_Conversion then
--                    log ("emit to view conversion subprog: '" & to_String (the_Subprogram.Name) & "'");
                  declare
                     class_Type : constant ada_Type.view := ada_Type.view (the_Subprogram.return_Type);
               ...
               elsif the_Subprogram.is_to_pointer_Conversion then
--                    log ("emit to pointer conversion subprog: '" & to_String (the_Subprogram.Name) & "'");
                  declare
                     class_Type : constant ada_Type.view := ada_Type.view (the_Subprogram.Parameters.Element (1).my_Type);
                     class_Name : constant String         := to_String (class_Type.declaration_Package.Name);
                  begin
                  ...
                  begin
--                       log ("emit subprog: '" & to_String (the_Subprogram.Name) & "'");

                     if         the_subprogram.access_Mode /= ada_subProgram.private_Access
                       and not (         the_subprogram.is_Destructor
                                and then is_interface_Type (Self.cpp_class_Type))    -- tbd: destructors and interface types together upsets vTable.
                     then
                        ...
                        append (spec_Source,  specification_Source (Self                => the_subprogram,
                                                                    declaration_package => Self.all'Access,
                        ...
                        append (spec_Source_private,  the_subprogram.pragma_import_Source (declaration_package  => Self.all'Access,
                                                                                           unique_function_name => unique_function_Name,
                                                                                           in_cpp_Mode          => in_cpp_Mode));
                     end if;

--                       log ("done subprog: '" & to_String (the_Subprogram.Name) & "'");
                  end;
               end if;
            end;
      ...
      --        -- emit Pointers    tbd: this causes probs with cpp class types, needs further investigation.
      --        --
      --        declare
      ...
      --              the_Type := Element (Self.a_Types,  Each);
      --
      --              if the_Type.my_array_Type /= null then
      --                 append (spec_Source, NL & NL
      --                                         & "   " & the_Type.Name & "_default_Terminator : " & the_Type.Name & ";" & NL & NL
      --                                         & "   package " & the_Type.Name & "_Pointers is new interfaces.c.Pointers (index              => interfaces.c.size_t," & NL
      --                                         & "                                                  element            => " & the_Type.Name & "," & NL
      --                                         & "                                                  element_array      => " & the_Type.my_array_Type.Name & "," & NL
      --                                         & "                                                  default_terminator => " & the_Type.Name & "_default_Terminator);" & NL & NL
      --                                         & "   subtype " & the_Type.Name & "_Pointer is " & the_Type.Name & "_Pointers.Pointer;");
      --              end if;
      --
      --           end loop;
      --        end;
      ...
      --  emit ic_pointer types
      --

      --  tbd: this causes probs with cpp class types: pragma CPP_Class makes the ada tagged record type 'limited',
      --      which is refused by interfaces.c.Pointers (which only takes a non-limited type)
      --

--        log ("emit ic_pointer types");

      emit_ic_pointer_Types :
      declare
         the_Type    : ada_Type.view;
      begin
         for Each in 1 .. Natural (Length (ic_pointer_Types)) loop
      ...
      --        log ("SPEC SOURCE SPRIV: '" & spec_Source_private & "'");

      if spec_Source_private /= "" then
      ...
      --        if Self.a_is_global_Namespace then
      append (spec_Source,  NL & NL & "end " & Self.qualified_Name & ";");
      --        else
      --           append (spec_Source,  "end " & namespace_Prefix & Self.qualified_Name & ";");
      --        end if;




      --  log ("writing the file:  namespace_Prefix: '" & namespace_Prefix & "'    qualified_Name: '" & Self.qualified_Name & "'");
--        log ("writing the file:    qualified_Name: '" & Self.qualified_Name & "'");
      ...
      begin
         ...
         --           if Self.a_is_global_Namespace then
         --              append (lowcase_Name,  Self.Name);
         --           else
         append (lowcase_Name,  Self.qualified_Name);
         --  append (lowcase_Name,  namespace_Prefix & Self.qualified_Name);
         --           end if;
         ...
         begin
            ...
            close  (the_File);
            --              log ("FILENAME: ' written and closed " & file_Name & "'");
         exception
